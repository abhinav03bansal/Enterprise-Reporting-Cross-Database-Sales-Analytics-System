...
MySQL:
bashpython insert_mysql.py
PostgreSQL:
bashpython insert_postgres.py
These scripts will:
...
bashpython root_cause_analysis.py
RCA Features:
...
Data Generation: Faker creates realistic synthetic data
Database Loading: Pandas + executemany bulk inserts
Extraction: Parallel extraction from MySQL and PostgreSQL
Transformation: Consolidation, deduplication, enrichment
Validation: Comprehensive quality checks and logging
Loading: Output to Tableau-ready format
Visualization: Tableau consumes enriched dataset
...
Data Generation: Run once to create baseline datasets
Database Setup: Create databases before insertion
ETL Execution: Run validation before and after ETL
Tableau Usage: Use extracts for datasets > 1M rows
Testing: Run tests after any code modifications
Monitoring: Check logs for warnings and errors
Optimization: Apply indexing before production use
